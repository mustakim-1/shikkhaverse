...
import { Bot, Send, Sparkles, X, Mic, MicOff, Maximize2, Minimize2, MessageSquare } from 'lucide-react';
...
export const GlobalAIAssistant: React.FC<GlobalAIAssistantProps> = ({ currentUser: propUser }) => {
  ...
  useEffect(() => {
    ...
    if (user) {
      // Check if we need to reset count based on date
      ...
      if (savedUsage) {
        ...
        if (date !== today) {
          // New day, reset count
          const updatedUser = { ...user, aiUsageCount: 0 };
          dataService.updateUser(updatedUser);
          setDailyCount(0);
          localStorage.setItem('ai_usage', JSON.stringify({ date: today, count: 0 }));
        } else {
          setDailyCount(user.aiUsageCount || 0);
        }
      } else {
        setDailyCount(user.aiUsageCount || 0);
      }
    ...
    try {
      ...
      if (currentUser) {
        ...
        if (currentUser.academicRecord) {
          const weakAreas = currentUser.academicRecord.weakAreas?.map(w => (w as any).topic).join(', ');
      ...
      // Update user in dataService to persist usage
      const user = dataService.getCurrentUser();
      if (user) {
  ...
  return (
    <div className={`fixed bottom-6 right-6 w-[350px] md:w-[400px] bg-[#0f172a] border border-slate-700/50 rounded-2xl shadow-2xl flex flex-col transition-all z-[100] ${isMinimized ? 'h-16' : 'h-[500px]'}`}>
      {/* Header */}
      <div className="p-4 border-b border-slate-700/50 flex items-center justify-between bg-slate-900/50 rounded-t-2xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-white">Lumina Mentor</h3>
            <p className="text-[10px] text-slate-400">Academic Assistant</p>
          </div>
        </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-slate-800/50 border border-slate-700 rounded-xl rounded-bl-none p-3 flex items-center gap-2">
          ...
          <div className="p-4 border-t border-slate-700/50 bg-slate-900/30">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-[10px] text-slate-500">
                Daily Limit: <span className={dailyCount >= DAILY_LIMIT ? 'text-red-500' : 'text-emerald-500'}>{dailyCount}/{DAILY_LIMIT}</span>
              </span>
            </div>
            <div className="relative flex items-center gap-2">
              <input
                ...
                className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all pr-20"
              />
              <div className="absolute right-1.5 flex items-center gap-1">
                <button
                  onClick={toggleVoice}
                  className={`p-1.5 rounded-lg transition-colors ${isListening ? 'bg-red-500/20 text-red-400' : 'text-slate-400 hover:bg-slate-700'}`}
                >
